...
This repository uses Codecov to collect coverage reports for both Python and C++ code. The CI workflow uploads:

- C++ coverage (lcov) captured from compiled binaries and uploaded as the `cpp` flag.
- Python coverage XML files generated by `pytest-cov` and uploaded per-package (each file is uploaded with a package-named flag).

Badges
- Overall Codecov badge (example included at the top of this file) shows project-level coverage for the `main` branch.
- For per-package badges, after Codecov has received coverage reports with flags (for example `core_pkg`), create a badge in the Codecov UI and use the provided markdown. In Codecov: `Settings → Repository → Badges` to generate SVG badge links for specific flags or branches.

If your repository is private, add the `CODECOV_TOKEN` secret in Settings → Secrets → Actions so the CI uploader can authenticate.

If you want me to add the generated badge(s) for specific packages or add a coverage shield for a different branch, tell me which package names or branch to target and I will add them to the README.
